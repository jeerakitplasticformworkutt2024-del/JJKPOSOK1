    ...
    <div className="space-y-2">
      {label && (
        <div className="flex justify-between items-center select-none">
          <label className="block text-xs font-extrabold text-[#7F1D1D] uppercase tracking-wide">
            {label}
          </label>
          {hasSigned && (
            <span className="text-[10px] text-emerald-600 font-extrabold flex items-center gap-1">
              <ShieldCheck size={12} className="stroke-emerald-600" />
      ...
      <div 
        ref={containerRef}
        className="relative border border-slate-300 bg-slate-50 rounded-xl overflow-hidden hover:border-[#7F1D1D] transition-colors shadow-inner"
      >
        <canvas
          ref={canvasRef}
          ...
          className="w-full h-[140px] cursor-crosshair bg-white block"
          style={{ touchAction: 'none' }}
        />
        
        {!hasSigned && placeholder && (
          <div className="absolute inset-0 pointer-events-none flex items-center justify-center text-center text-[11px] text-slate-400 font-medium px-6 leading-relaxed">
            {placeholder}
          </div>
        )}
        ...
        {hasSigned && (
          <button
            ...
            className="absolute bottom-2 right-2 p-1.5 px-2 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 rounded-lg flex items-center gap-1 text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer"
          >
            <RotateCcw size={10} />
